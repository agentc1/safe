#!/usr/bin/env python3
"""Run the PR11.5 statement ergonomics gate."""
...
from _lib.pr115_surface import (
    corpus_paths,
    negative_cases,
    negative_paths,
    positive_cases,
...
DEFAULT_REPORT = REPO_ROOT / "execution" / "reports" / "pr115-statement-ergonomics-report.json"


def assert_source_fragments(path: Path, fragments: tuple[str, ...]) -> list[str]:
...
def run_positive_case(
    *,
    ...
    source_fragments = assert_source_fragments(source, expected_source_fragments)
...
def generate_report(*, env: dict[str, str], scratch_root: Path | None = None) -> dict[str, Any]:
    ...
    with managed_scratch_root(scratch_root=scratch_root, prefix="pr115-ergonomics-") as temp_root:
    ...
    return {
        ...
        "status": "ok",
        "syntax_policy": {
            "optional_semicolons_parser_side": True,
            "lexer_token_stream_changed": False,
            "var_is_additive": True,
            "legacy_statement_local_declarations_retained": True,
            "deferred_to_pr118b": [
...
def main() -> int:
    ...
    report = finalize_deterministic_report(
        ...
        label="PR11.5 statement ergonomics",
    )
    write_report(args.report, report)
    print(
        "PR11.5 statement ergonomics report written to"
...
if __name__ == "__main__":
    try:
        ...
        print(f"pr11.5 statement ergonomics: ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)

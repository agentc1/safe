#!/usr/bin/env python3
"""Run the PR11.6.2 legacy Ada syntax removal gate."""
...
from _lib.pr1162_surface import (
    corpus_paths,
    migration_examples,
    negative_cases,
    negative_paths,
...
from migrate_pr1162_legacy_syntax import rewrite_safe_source


DEFAULT_REPORT = REPO_ROOT / "execution" / "reports" / "pr1162-legacy-ada-syntax-removal-report.json"
...
def assert_source_surface(
    ...
    *,
    ...
    for fragment in forbidden:
        require(fragment not in normalized, f"{path.name}: source still contains removed fragment {fragment!r}")
    return {
...
def generate_report(*, env: dict[str, str], scratch_root: Path | None = None) -> dict[str, Any]:
    ...
    with managed_scratch_root(scratch_root=scratch_root, prefix="pr1162-legacy-") as temp_root:
    ...
    return {
        ...
        "syntax_policy": {
            ...
            "removed_source_spellings": [
                "aliased",
            ],
            "empty_suites_replace_null_statements": True,
            "removed_words_remain_reserved": True,
        },
...
def main() -> int:
    ...
    report = finalize_deterministic_report(
        ...
        label="PR11.6.2 legacy Ada syntax removal",
    )
    write_report(args.report, report)
    print(f"PR11.6.2 legacy Ada syntax removal: OK ({display_path(args.report, repo_root=REPO_ROOT)})")
    return 0

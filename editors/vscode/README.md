...
through the current PR11.8c compiler surface.
...
- The grammar statically highlights the current PR11.8c lowercase surface,
  ...
  surface with `<<` / `>>` shifts and plain `and` / `or` / `xor`. Mixed-case
  Safe spellings are highlighted as invalid.

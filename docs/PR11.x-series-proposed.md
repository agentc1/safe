...
PR11.7 landed as a decision milestone rather than a default syntax cutover.
It isolated the highest-risk reference-surface proposals, ran them against a
fixed ownership/reference corpus, and recorded separate outcomes for each.

### Proposals Included

| Syntax Proposal | Summary |
|-----------------|---------|
| Capitalisation as Reference Signal | Experimented behind `--experiment pr117-reference-signal`; deferred because it requires case-significant user-name resolution plus migration/tooling churn |
| Implicit Dereference | Deferred as a stronger surface recommendation; the fixed corpus still exposes emitted-Ada divergence from explicit `.all` spellings even though partial implementation already exists |
...
- An experiment-only compiler path and deterministic gate/report for the
  reference-signal prototype without changing the default admitted language
  surface.
- A fixed technical and readability corpus that compares baseline,
  implicit-dereference, and combined experiment variants.
- A clear decision for each in-scope proposal: reference-signal casing
  deferred, and implicit dereference also deferred as a future tightening
  target rather than blessed as the parity-clean reference surface.
...
| Capitalisation as Reference Signal | PR11.7 (deferred) |
| Implicit Dereference | PR11.7 (deferred as a stronger surface recommendation) |

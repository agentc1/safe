...
Add a built-in `print` function so Safe programs can produce visible output
without handwritten Ada driver code.

### Scope

- `print` is a built-in function that accepts `integer`, `string`, or
  `boolean` and writes a line to standard output.
...
- The emitter lowers `print` calls through a generated `safe_io.ads`
  ...
  side effect from the proved code surface:
    ...
    - The emitted package body `with`s `Safe_IO` (not `Ada.Text_IO`
      ...
      conversions for non-string arguments.

...
- Parser support for the new spellings and unified `function` keyword, with a
  clear coexistence or migration rule for legacy forms.
- Mechanical migration strategy for the existing test corpus and Rosetta corpus,
  including `procedure` → `function` conversion.
...
- The emitter maps `function` with no `returns` clause to Ada `procedure` and
  `function` with a `returns` clause to Ada `function`, keeping emitted Ada
  unchanged.
...
| Task Channel Direction Constraints | Not scheduled (post-PR11.x concurrency ergonomics) |

...
- Prove the emitted fixtures admitted by `PR11.3` discriminated-record,
  discriminant-constraint, tuple, destructuring, tuple-channel, and builtin
  `result` structured-return support.
- Revalidate any previously proved sequential fixtures whose emitted proof
  shape is affected by parser, resolver, tuple, or discriminant-emission
  changes.
...
- A dedicated deterministic proof gate/report/CI job with a non-shrinkable
  ...
  fixture set, including the structured-return surfaces.
...
the admitted `PR11.3` discriminant/tuple corpus forces broader scope-exit or

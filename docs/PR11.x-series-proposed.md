...
- PR11.8f.1 follows PR11.8f-b (bounded traversal emission; removes Skip_Proof scaffolding).
- PR11.8g depends on both PR11.8f.1 and PR11.8b.
- PR11.8h follows PR11.8g (safe prove CLI command).
...
- Contracts and invariants first; GNATprove annotations allowed only for
  timeout-only leftovers and the explicitly tracked traversal `Skip_Proof`.
- All Safe code proves except IO. The traversal `Skip_Proof` is temporary
  scaffolding removed in PR11.8f.1, not permanent debt.
...
## PR11.8f.1: Bounded Traversal Emission

Remove the `Skip_Proof` scaffolding introduced in PR11.8f-b by changing the
emitter so that self-recursive traversal lowers to bounded iteration instead
of a while-pointer-chase loop.

### Scope

- The compiler already knows the inferred reference structure is acyclic (it
  inferred the reference root from a direct self-recursive record, not an
  arbitrary graph). That knowledge must be lowered into the emitted Ada as a
  bounded `for` loop with a compiler-generated length cap, not a `while`
  loop chasing pointers.
- Once the emitter produces bounded traversal, GNATprove proves termination
  structurally and the body-only `Skip_Proof` annotations from PR11.8f-b are
  deleted.
...
The `Skip_Proof` annotations in PR11.8f-b are deleted when and only when:

1. The emitter produces bounded iteration for every self-recursive traversal
   that previously used a while-pointer-chase loop.
2. GNATprove proves termination of the bounded form without annotations.
3. The affected fixtures (`rule4_linked_list`, `rule4_linked_list_sum`, and any
   other traversal fixtures) pass the blocking proof gate without `Skip_Proof`.
...
Follows PR11.8g. Must land before PR11.9 so that the artifact contract freeze

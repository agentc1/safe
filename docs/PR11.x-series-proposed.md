...
convention for all user-defined identifiers.

### Rule

All user-defined identifiers — package names, function names, type names,
subtype names, variable names, parameter names, record field names, channel
names, and task names — must be fully lowercase with underscores as word
separators. The compiler rejects any user-defined identifier containing an
uppercase letter.

Reserved words, predefined type names (`integer`, `string`, `boolean`), and
predefined function names (`ok`, `fail`) are already lowercase and unaffected.
...
- The convention is enforceable mechanically with a single resolver check
  and a straightforward migrator.

### Deliverables

- Compiler enforcement: the resolver rejects any user-defined identifier
  containing an uppercase letter, with a targeted diagnostic.
- A mechanical migrator that lowercases all user-defined identifiers across
  the corpus.
...
- All user-defined identifiers are lowercase (enforced by PR11.7). No
...
all-lowercase identifier convention now that the language surface is

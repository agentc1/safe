# PR11.x Series — Milestone Roadmap

This document is the detailed roadmap for the tracked `PR11.x` planned
milestones.

It is wired into `execution/tracker.json` and `execution/dashboard.md` as a
future-planned series, but it is not yet wired into CI, gate scripts, or
committed evidence reports. No milestone described here is in progress or done.
...
respectively. All four descend from PR10.1.

Dependency chain: PR10.1 → PR10.3 / PR10.4 / PR10.5, with PR10.6 depending on
PR10.3 and PR11.1 depending on PR10.4, PR10.5, and PR10.6.
...
### Proof-Coverage Deferral

Emitted proof expansion beyond the completed `PR10.x` proof corpus is not part
of the `PR11` language-iteration series. It re-enters only after `PR11.11` or
as a separate parallel track once the admitted language surface has stabilized.
...
Front-load the lowest-risk syntax admissions first so the surface can stabilise
...
possibly `var`) so the most plausible admissions stabilise early.
...
**Artifact contracts before library** — PR11.9 stabilises machine interfaces
before PR11.10 introduces library types that external tools will consume.
Library consumers need stable interfaces.
...
| PS-029 | Deallocation semantics (TBD-11) | Post-PR11.11 (v1.0 baseline) |
...
- `PR11.9` through `PR11.11`: if tooling, library, or generic claims require
  `PS-024`, `PS-025`, `PS-027`, or `PS-029`, record them as post-`PR11.11`
...
3. **When should post-`PR10.x` emitted proof expansion re-enter active
   planning?** The default answer is after `PR11.11` or as a separate parallel
   track once the admitted language surface stabilizes, but that handoff point
   is still open.
